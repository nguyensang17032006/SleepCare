...
class QuestionnaireViewModel extends ChangeNotifier {
  ...
  bool validateQ5() {
    final requiredKeys = ['a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i'];
    for (var k in requiredKeys) {
      if (q5[k] == null) return false;
    }

    if (q5jOtherReason.trim().isNotEmpty && q5['j'] == null) {
      return false;
    }
  ...
  bool validateQ6To10() {
    ...
    if (q10HasPartner == 0) {
      final requiredKeys = ['a', 'b', 'c', 'd'];
      for (var k in requiredKeys) {
        if (q10[k] == null) return false;
      }
      if (q10eOtherReason.trim().isNotEmpty && q10['e'] == null) {
        return false;
      }
    }
  ...
  bool calculatePSQI() {
    ...
    int q2Score = 0;
    if (sleepLatencyMinutes! <= 15) {
      q2Score = 0;
    } else if (sleepLatencyMinutes! <= 30) {
      q2Score = 1;
    } else if (sleepLatencyMinutes! <= 60) {
      q2Score = 2;
    } else {
      q2Score = 3;
    }
  ...
  Future<bool> submitQuestionnaire() async {
    ...
    try {
      ...
      debugPrint(" LỖI TẠI ONBOARDING SUBMIT: $e");
